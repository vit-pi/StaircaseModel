...
Preprocessing (works on Linux and requires supercomputing cluster operated by SLURM for efficiency):
1) Install GNU C Compiler, for example with a command: module load gcc-9.3.0
2) Compile the files, for example with the code: g++ -std=c++11 -O3 -march=skylake MMainFile.cpp Lattice.cpp Functions.cpp
3) Create a submission bash file for SLURM. Often job arrays are needed.

Postprocessing (works on Windows):
