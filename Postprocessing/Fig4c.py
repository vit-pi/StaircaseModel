...
for row in range(2):
    for col in range(2):
        ...
        ax = axs[row, col]
        gen_space_tot = data_set.wild_type_plot((death, switch_params[row], move_params[col][0], move_params[col][1]), run_index[row][col], 0, 0, True, LD[row][col], time_type[row][col], ax, x_labels[row][col], pop_labels[row][col], r_labels[row][col], False)
        df = pd.DataFrame(gen_space_tot[0])
        df.to_csv("SourceData/Fig4c_pop0_row" + str(row) + "_col" + str(col) + ".csv", index=False)
plt.show()
