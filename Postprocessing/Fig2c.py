...
min_nu_exp = np.log10(1e-5/2)
max_nu_exp = np.log10(10*2)
nu_num = 57
LDs = [1, 3, 6]
...
for col in range(2):
    ...
    if crit_mot is None:
        ...
        ax.text(motilities[0]/2, 4 * min(minT1, minD), "Low Motility", fontsize=15, color='green', rotation=90)
    ...
    ax.set_ylim([min(minD, minT1)/2, max(maxT1, maxD)*2])
...
labels = ["$T_{evo} (R=1)$", "$T_{eco} (R=1)$", "$T_{evo} (R=3)$", "$T_{eco} (R=3)$", "$T_{evo} (R=6)$", "$T_{eco} (R=6)$"]
