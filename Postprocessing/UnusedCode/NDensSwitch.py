    ...
    for row in range(2):
        for col in range(2):
            ...
            if switch_param is not None:
                [min, max, im] = data_set.dens_switch_heatmap(death, switch_param, LD, time_type, fig, ax, min_max, colorbar, labels)
                if min < minimum:
                    minimum = min
                if max > maximum:
                    maximum = max
...
cbar.set_label("adaptation rate $a_g$ (1/h)", fontsize=12, rotation=90)
print(min_max)
plt.show()
