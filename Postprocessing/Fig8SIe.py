    ...
    for row in range(2):
        for col in range(2):
            ...
            if switch_param is not None:
                ...
                df_a.to_csv("SourceData/Fig8SIe_adap_row" + str(row) + "_col" + str(col) + ".csv", index=False)
                df_s.to_csv("SourceData/Fig8SIe_stds_row" + str(row) + "_col" + str(col) + ".csv", index=False)
