...
def sdeath_plot(ax):
    ...
    [min, max, adap_rates, stds, im] = data_set_sdeath.stressdeath_heatmap(LD, time_type, fig, ax, min_max, False, labels)
...
def chem_plot(ax):
    chem_folder_paths = ["AdapRateChemotaxV" + str(i + 1) + "/" for i in range(sim_num)]
    chem_type = [1 for i in range(sim_num)]
    zip = ["Build/NewBuildV" + str(i + 1) + ".zip" for i in range(sim_num)]
    data_set_chem = ad.AdapDatabase(chem_folder_paths, chem_type, zip)
    [min, max, adap_rates, stds, im] = data_set_chem.chemotax_heatmap(LD, time_type, fig, ax, min_max, True, labels)
    df_a = pd.DataFrame(adap_rates)
    df_s = pd.DataFrame(stds)
    df_a.to_csv("SourceData/Fig6SI_adap.csv", index=False)
    df_s.to_csv("SourceData/Fig6SI_stds.csv", index=False)

# Make the plot
fig, axs = plt.subplots(ncols=2, nrows=1, figsize=(10, 3.5), constrained_layout=True)
#sdeath_plot(axs[0])  # would generate Fig5SI at the same time
chem_plot(axs[1])
plt.show()