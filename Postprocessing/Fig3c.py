...
for row in range(2):
    for col in range(3):
        ...
        ax = axs[row, col]
        gen_space_tot = data_set.wild_type_plot((move_params[col][0], move_params[col][1], switch_params[row]), run_index[row][col], 0, 1-row, True, LD, 0, ax, x_labels[row][col], pop_labels[row][col], r_labels[row][col], True)
        df = pd.DataFrame(gen_space_tot[0])
        df.to_csv("SourceData/Fig3c_pop0_row" + str(row) + "_col" + str(col) + ".csv", index=False)
        df = pd.DataFrame(gen_space_tot[1])
        df.to_csv("SourceData/Fig3c_pop1_row" + str(row) + "_col" + str(col) + ".csv", index=False)
plt.show()
