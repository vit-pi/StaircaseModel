...
int main(int argc, char **argv) {
    ...
    int AdapNum = 7;
    vector<double> hgt_params{1E-7, 3.16E-7, 1E-6, 3.16E-6, 1E-5, 3.16E-5, 1E-4, 3.16E-4, 1E-3};
    vector<double> move_params{1E-5, 3.16E-5, 1E-4, 3.16E-4, 1E-3, 3.16E-3, 1E-2, 3.16E-2, 1E-1, 3.16E-1, 1, 3.16, 10};
    vector<string> hgt_names{"1E7", "3E7", "1E6", "3E6", "1E5", "3E5", "1E4", "3E4", "1E3"};
    vector<string> move_names{"1E5", "3E5", "1E4", "3E4", "1E3", "3E3", "1E2", "3E2", "1E1", "3E1", "1", "3", "10"};
    // Compute the indices
    int mov_num = move_params.size();
    int hgt_index = task_id/(Repeat*mov_num);
    int move_index = task_id % mov_num;

    // Set parameters
	PProp[0].Move = move_params[move_index];
    PProp[0].ConsiderHGT = true;
    PProp[0].DeathRate = 3E-1;  // if HGTDeadly
    PProp[0].HGTRate = hgt_params[hgt_index];
    string FileSpecifier = to_string(task_id)+"_M"+move_names[move_index]+"_HGT"+hgt_names[hgt_index];
	// Run AdapImage
